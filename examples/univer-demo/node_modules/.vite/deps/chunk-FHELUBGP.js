...
var getDecimalFromPattern = (pattern, defaultValue = 0) => {
  ...
  return (_info$maxDecimals = lib_exports.getFormatInfo(pattern).maxDecimals) !== null && _info$maxDecimals !== void 0 ? _info$maxDecimals : defaultValue;
};
...
var getPatternPreview = (pattern, value, locale = "en") => {
  try {
    ...
    const result = lib_exports.format(pattern, value, {
      locale,
      throws: false
    });
    if (value < 0) return {
...
var TEXT_FORMAT_MARK = { tl: {
  size: 6,
  color: "#409f11"
} };
var SheetsNumfmtCellContentController = class SheetsNumfmtCellContentController2 extends Disposable {
  ...
  _initInterceptorCellContent() {
    ...
    this.disposeWithMe(this._instanceService.getCurrentTypeOfUnit$(UniverType.UNIVER_SHEET).pipe(switchMap((workbook) => {
...
var UniverSheetsNumfmtPlugin = class UniverSheetsNumfmtPlugin2 extends Plugin {
...
_defineProperty(UniverSheetsNumfmtPlugin, "type", UniverType.UNIVER_SHEET);
...
var getCurrencyOptions = () => currencySymbols.map((item) => ({
  label: item,
  value: item
}));
...
var getDateFormatOptions = () => DATEFMTLISG.map((item) => ({
  label: item.label,
  value: item.suffix
}));
var getNumberFormatOptions = () => NUMBERFORMAT.map((item) => ({
  label: item.label,
  value: item.suffix,
  color: item.color
}));
...
//# sourceMappingURL=chunk-FHELUBGP.js.map
