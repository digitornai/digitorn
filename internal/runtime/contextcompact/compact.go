...
func Summarize(ctx context.Context, msgs []sessionstore.Message, keepRecent int, s Summarizer, maxTokens int, goal, priorSummary string) Result {
	...
	if strings.TrimSpace(priorSummary) != "" {
		pri := sessionstore.Message{
			...
			Parts:   []sessionstore.MessagePart{{Type: sessionstore.PartTypeText, Text: priorSummary}},
		}
...
// duplicating it.
const (
	compactionIntro = "Context checkpoint — the earlier conversation was compacted to stay within the token window. The recap below is the AUTHORITATIVE record of everything that happened before this point. Treat it as ground truth and continue exactly where you left off, as if the full history were still in view — the user must not be able to tell a compaction occurred. Your durable working memory (goal, tasks, key facts) is injected separately and is still in force; rely on it."
	compactionOutro = "Resume the mission directly: continue as if no compaction happened. Do not re-introduce yourself, do not restate this recap to the user, and do not ask the user to repeat anything captured above."
)
