...
func Open(file string) *Index {
	mm := mmap(file)
	// If parsing a corrupt index panics (corrupt()/fatal) AFTER the mmap, unmap it
	// before the panic propagates — otherwise the failed Open leaks the mapping and
	// (on Windows) leaves the file locked. Added to the vendored library to make the
	// caller's recover()-then-fall-back path leak-free.
	ok := false
	defer func() {
		if !ok {
			_ = mm.close()
...
// NumFiles returns the number of files in the index. Added to the vendored
// library so a caller can enumerate every indexed name (fileid 0..NumFiles-1)
// and diff it against the set it intended to index — the difference is the set
// of files the indexer silently skipped (invalid UTF-8, over-long lines, …),
// which the caller must always scan to keep search results complete.
...
type mmapData struct {
	f *os.File
	d []byte
	h uintptr // Windows file-mapping handle ; 0 on unix (kept so Close can release it)
}

// Close unmaps the index and releases the underlying file handle. Added to the
// vendored library : upstream codesearch never unmaps (it relies on process
// exit), which would leak one mmap + descriptor per rebuild in a long-running
// daemon. Safe to call once ; the Index must not be used afterwards.
