...
// Close releases the index file handle after Flush. Added to the vendored
// library : upstream relies on process exit to close it, but a long-running
// daemon must close explicitly so the finished index can be renamed (Windows
// forbids renaming an open file) and so descriptors are not leaked per build.
func (ix *IndexWriter) Close() error {
	if ix.main != nil && ix.main.file != nil {
		return ix.main.file.Close()
	}
	return nil
}
...
func copyFile(dst, src *bufWriter) {
	...
	if err != nil {
		fatalf("copying %s to %s: %v", src.name, dst.name, err)
	}
}
...
func (ix *IndexWriter) flushPost() {
	...
	if n, err := w.Write(data); err != nil || n < len(data) {
		...
		fatalf("short write writing %s", w.Name())
	}
