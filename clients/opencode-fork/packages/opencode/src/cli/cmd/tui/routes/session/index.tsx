            ...
            <Show
              ...
              fallback={
                <box flexGrow={1} minHeight={0} justifyContent="center" alignItems="center">
                  <Spinner color={theme.textMuted}>Loading session…</Spinner>
                </box>
              }
            >
              <scrollbox
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
  ...
  return (
    <Show when={!shouldHide()}>
      {/* Digitorn: ONE uniform renderer for every tool (✓/spinner, Name param,
          result) instead of opencode's per-tool icons. Stock opencode untouched. */}
      <Show when={Boolean(process.env.DIGITORN_URL)} fallback={
        <Switch>
...
// Digitorn's single uniform tool renderer: spinner while running, ✓ when done
// (✗ on error), "Name param (result)" — same shape for EVERY tool.
function UniformTool(props: ToolProps<any>) {
  const { theme } = useTheme()
  ...
  const param = createMemo(() => {
    const i = (props.input ?? {}) as Record<string, any>
  ...
  const result = createMemo(() => {
    const m = (props.metadata ?? {}) as Record<string, any>
    const n = typeof m.count === "number" ? m.count : typeof m.matches === "number" ? m.matches : undefined
    return n != null ? `${n} match${n === 1 ? "" : "es"}` : ""
  })
  return (
    <InlineTool
      icon={failed() ? "✗" : "✓"}
      // Running tools take an accent so they read as live machine work, distinct
      // from the agent's white prose AND from the dim (textMuted) completed state.
      color={running() ? theme.primary : undefined}
      pending={name() + "…"}
      complete={!running()}
      spinner={running()}
      part={props.part}
    >
      {name()}
      {/* The name carries the action + state color; the param (path/pattern/…) and
          result sit a tone back in textMuted, so a row layers name › param › result
          instead of reading as one flat block. */}
      <Show when={param()}>
        <span style={{ fg: theme.textMuted }}> {param()}</span>
      </Show>
      <Show when={result()}>
        <span style={{ fg: theme.textMuted }}> ({result()})</span>
      </Show>
    </InlineTool>
  )
}
...
function Task(props: ToolProps<typeof TaskTool>) {
  ...
  // Same layering as the tool rows : the NAME (sub-agent type, current tool)
  // carries the state color, while its detail (description, tool param, counts)
  // sits a tone back in textMuted — so a sub-agent block reads name › detail
  // instead of one flat color.
  const muted = (s: any) => <span style={{ fg: theme.textMuted }}>{s}</span>
  const body = createMemo(() => {
    if (!props.input.description) return null
    const description =
    ...
    if (isRunning() && retrying) {
      work = muted(`\n↳ ${Locale.truncate(retrying.message, 80)} [retrying attempt #${retrying.attempt}]`)
    } else if (isRunning() && tools().length > 0) {
      if (current()) {
        const state = current()!.state
        const title = state.status === "running" || state.status === "completed" ? state.title : undefined
        // Distinct 3rd tone : the ↳ tool NAME in theme.text (so it differs from
        // the header's state color AND from the muted param), its param muted.
        work = (
          <>
            <span style={{ fg: theme.text }}>{`\n↳ ${Locale.titlecase(current()!.tool)}`}</span>
            {muted(` ${title}`)}
          </>
        )
