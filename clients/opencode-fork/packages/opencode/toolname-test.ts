...
const c:any = await daemonFetch(cfg, `/api/apps/${app}/sessions`, {method:"POST", body:JSON.stringify({title:"toolname", workdir:process.cwd()})})
...
await f(`${cfg.url}/session/${sid}/prompt_async` as any,{method:"POST",body:JSON.stringify({parts:[{type:"text",text:"Analyse ce projet : lance plusieurs lectures/recherches en parallèle puis une commande bash, et résume."}],agent:"build"})})
console.log("sent, 22s…"); await new Promise(r=>setTimeout(r,22000))
const tools=[...parts.values()]
const names=[...new Set(tools.map(t=>t.tool))]
console.log("tool names seen:", names.join(", "))
const raw=names.filter(n=>/_|\./.test(n) && !["run_parallel"].includes(n))
console.log(raw.length? "✗ RAW NAMES still: "+raw.join(", ") : "✓ all clean verbs (no module-prefixed/underscore leftovers)")
const rp=tools.find(t=>t.tool==="run_parallel"); console.log("run_parallel children:", rp?.state?.metadata?.children?.length ?? "none")
process.exit(0)
