...
const c:any=await daemonFetch(cfg,`/api/apps/${app}/sessions`,{method:"POST",body:JSON.stringify({title:"rpflat",workdir:process.cwd()})})
...
await f(`${cfg.url}/session/${sid}/prompt_async` as any,{method:"POST",body:JSON.stringify({parts:[{type:"text",text:"Lance run_parallel avec 3 filesystem.glob (patterns différents) puis résume."}],agent:"build"})})
await new Promise(r=>setTimeout(r,22000))
const all=[...parts.values()].sort((a,b)=>a.id<b.id?-1:1)
console.log("has run_parallel part:", all.some(p=>p.tool==="run_parallel")?"YES (bad)":"NO ✓")
console.log("tool parts:")
for(const p of all) console.log(`  ${p.id.slice(-12)} ${p.tool.padEnd(8)} ${p.state?.status} ${JSON.stringify(p.state?.input||{}).slice(0,30)}`)
process.exit(0)
