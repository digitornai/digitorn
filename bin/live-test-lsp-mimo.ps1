...
$bundleDir = Join-Path $repoRoot "bin\test-apps\lsp-probe-mimo"
...
try {
    ...
    Log-Info "installing lsp-probe-mimo ..."
    $installResp = Invoke-Rest "POST" "$baseURL/api/apps/install" $headers (@{ source = $bundleDir } | ConvertTo-Json)
    if ($installResp.app_id -ne "lsp-probe-mimo") { throw "install : app_id=$($installResp.app_id)" }
    Log-Ok "installed app_id=$($installResp.app_id)"

    $sessBody = @{ workdir = ($workdir -replace '\\','/') } | ConvertTo-Json
    $sessResp = Invoke-Rest "POST" "$baseURL/api/apps/lsp-probe-mimo/sessions" $headers $sessBody
    ...
    $msgResp = Invoke-Rest "POST" "$baseURL/api/apps/lsp-probe-mimo/sessions/$sid/messages" $headers (@{ content = $question } | ConvertTo-Json)
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        try { $hist = Invoke-Rest "GET" "$baseURL/api/apps/lsp-probe-mimo/sessions/$sid/history" $headers $null } catch { continue }
        if ($hist.messages) {
    ...
    if (-not $assistantMsg) {
        ...
        try { (Invoke-Rest "GET" "$baseURL/api/apps/lsp-probe-mimo/sessions/$sid/history" $headers $null).messages | ForEach-Object { Write-Host "  [$($_.role)] $($_.content)" } } catch {}
