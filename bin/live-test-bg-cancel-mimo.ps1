# Live BG-CANCEL E2E worker end-to-end test (GATEWAY mode)
#
...
$bundleDir = Join-Path $repoRoot "bin\test-apps\bg-cancel-probe-mimo"
...
$base = "C:\Users\ASUS\AppData\Local\Temp\digitorn-live-bgcancel"
...
try {
    ...
    Log-Info "installing bg-cancel-probe-mimo ..."
    $installResp = Invoke-Rest "POST" "$baseURL/api/apps/install" $headers (@{ source = $bundleDir } | ConvertTo-Json)
    if ($installResp.app_id -ne "bg-cancel-probe-mimo") { throw "install : app_id=$($installResp.app_id)" }
    Log-Ok "installed app_id=$($installResp.app_id)"

    $sessBody = @{ workdir = ($workdir -replace '\\','/') } | ConvertTo-Json
    $sessResp = Invoke-Rest "POST" "$baseURL/api/apps/bg-cancel-probe-mimo/sessions" $headers $sessBody
    ...
    $msgResp = Invoke-Rest "POST" "$baseURL/api/apps/bg-cancel-probe-mimo/sessions/$sid/messages" $headers (@{ content = $question } | ConvertTo-Json)
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        try { $hist = Invoke-Rest "GET" "$baseURL/api/apps/bg-cancel-probe-mimo/sessions/$sid/history" $headers $null } catch { continue }
        if ($hist.messages) {
            foreach ($m in $hist.messages) {
                if ($m.role -eq "assistant" -and $m.content -and ($m.content -match "killed_at=")) { $assistantMsg = $m; break }
            }
        }
    ...
    if (-not $assistantMsg) {
        ...
        try { (Invoke-Rest "GET" "$baseURL/api/apps/bg-cancel-probe-mimo/sessions/$sid/history" $headers $null).messages | ForEach-Object { Write-Host "  [$($_.role)] $($_.content)" } } catch {}
